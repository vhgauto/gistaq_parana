...
El proyecto está gestionado mediante [`targets`](https://books.ropensci.org/targets/). Para ejecutar todas las funciones, correr en la consola:
...
```{r}
#| label: tbl-s2msi
#| tbl-cap: Propiedades de las bandas S2-MSI.
...
La <span style='color:`r c2`;'>fecha actual</span> y los siguientes 10 muestreos son:

```{r}
#| label: tbl-adq
#| tbl-cap: Fecha actual y próximos 10 muestreos
...
Sitio web desarrollado y mantenido por: Mgtr. Víctor Gauto [`r icono_mail`](`r link_mail_vhg`).
