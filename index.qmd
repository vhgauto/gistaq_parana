...
A continuación se describen el objetivo general ([$\S$-@sec-objetivos]) y específicos del presente proyecto. Se detallan las tareas que se ejecutan para la obtención, extracción y almacenamiento de datos espectrales y fisicoquímicos ([$\S$-@sec-ejecucion]). Se describe la metodología de los muestreos ([$\S$-@sec-muestreos]). Los resultados ([$\S$-@sec-resultados]) incluyen las relaciones entre parámetros y la evolución de éstos. Se visualizan las firmas espectrales por sitio y por muestreo ([$\S$-@sec-firmas-espectrales]). Se describen el desarrollo y selección de algoritmos ([$\S$-@sec-algoritmos]). La discusión de la bibliografía y los algoritmos encontrados se discuten en ([$\S$-@sec-discusion]). 
Al final se encuentran las referencias bibliográficas.

La finalidad de este desarrollo es reunir en un único sitio web las características principales del proyecto, registrar datos, visualizar resultados, evaluar algoritmos y que además sea utilizado como material de consulta general de la metodología.

:::

# Muestreos {#sec-muestreos}

Los muestreos, según lo permita la logística, se llevan a cabo en la fecha de adquisición de S2-MSI cada 5 días, sobre la región de interés, con cielo despejado. El plan de adquisición actualizado puede [descargarse](https://sentinels.copernicus.eu/web/sentinel/copernicus/sentinel-2/acquisition-plans) en formato `.kml`.
...
In situ, se miden pH, conductividad (cond.) y profundidad de disco de Secchi (SDD). Luego, en el laboratorio se analizan turbidez (turb.) y sólidos suspendidos totales (sól. susp.). A continuación se describe cada parámetro fisicoquímico:

* pH y conductividad (μS/cm), a partir de un equipo multiparamétrico.

* Profundidad de disco de Secchi (cm).

* Sólidos suspendidos totales (ppm) por gravimetría, utilizando el método estandarizado 2540 D, sólidos totales en suspensión secados a 103-105°C @baird2017standard, que emplea filtros de 47 mm de diámetro y 0,5 μm de poro.
...
Una definición más rigurosa @Chen2017 define a la reflectancia de sensado remoto ($R_{rs}(\lambda)$) como el cociente entre la radiancia saliente del agua verticalmente ascendente ($L_{w}(\lambda)$) y la irradiancia verticalemente descendente sobre la superficie del agua ($E_d(0^{+})$), a una longitud de onda $\lambda$, según la ecuación [-@eq-r-rs].
...
```{r}
lista_figura_evolucion_gis[[1]]
```

## `r fechas_gis_v[2]`

```{r}
lista_figura_evolucion_gis[[2]]
```

## `r fechas_gis_v[3]`

```{r}
lista_figura_evolucion_gis[[3]]
```

## `r fechas_gis_v[4]`

```{r}
lista_figura_evolucion_gis[[4]]
```

:::

<br>
Las curvas <b style='color:`r c1`'>color violeta</b> corresponden a los sitios cercanos a la orilla chaqueña. Los puntos próximos a la costa correntina son de <b style='color:`r c2`'>color verde</b>.
...
Las características de las bandas espectrales de S2-MSI se muestran en la tabla [-@tbl-s2msi]:
...
A continuación se indican (según el caso) las ecuaciones para la estimación de las propiedades, los sensores y longitudes de onda empleados, las características de los cuerpos de agua y la justificación de los autores, junto con la referencia bibliográfica.

Se pretende poder justificar los algoritmos desarrollados propios a partir de los obtenidos por otros autores.

Las longitudes de onda de las bandas en las siguientes expresiones se encuentran en la tabla [-@tbl-s2msi].
...
Esto origina una ejecución en cadena de múltiples funciones que permiten la obtención de datos espectrales y fisicoquímicos para la generación y almacenamiento de resultados.
...
Incluye la fecha, las coordenadas geográficas de los sitios de muestreo, los valores de pH, conductividad (μS/cm), profundidad de disco de Secchi (cm), sólidos suspendidos totales (mg/L) y turbidez, por dos métodos: colorimétrico (HACH, NTU) y turbidímetro (Hazemeter, EBC).

Al finalizar el procesamiento de cada nueva muestra de agua ([$\S$-@sec-muestreos]), se actualiza manualmente con los datos obtenidos. La ejecución del comando mostrado al inicio de esta sección ([$\S$-@sec-ejecucion]) genera la sucesión de todas las funciones siguientes.
...
Se ejecuta el script Python que descarga el producto. Utiliza las credenciales del desarrollador del presente sitio web en [Copernicus Data Space Ecosystem](https://dataspace.copernicus.eu/). El nivel de procesamiento es L2A, en reflectancia de superficie, con corrección atmosférica automática.

El producto a descargar corresponde al SAFE ([Standard Archive Format for Europe](https://earth.esa.int/eogateway/activities/safe-the-standard-archive-format-for-europe)) y se encuentra comprimido en un archivo `.zip`. El tamaño del producto es 1Gb aproximadamente.
...
Sobre el curso de agua se obtienen pH, conductividad, profundidad de disco de Secchi y las coordenadas geográficas. En laboratorio se miden turbidez por dos métodos: colorimétrico (HACH) y turbidímetro (Hazemeter), y sólidos suspendidos totales por gravimetría.
