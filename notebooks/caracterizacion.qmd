...
param_unid_v <- c( # <2>
  "<i>turb</i> (NTU)", "<i>secchi</i> (cm)", "<i>susp</i> (ppm)", # <2>
  "<i>cond</i> (μS/cm)", "pH" # <2>
) # <2>
...
```{r}
f_figura_evolucion_lab <- function(parametro) { # <1>
  g <- d |>
    ...
    geom_line_interactive( # <2>
      aes(data_id = interaction(fecha, param)),
      hover_nearest = TRUE,
      linewidth = 2, alpha = .8
    ) +
    geom_point_interactive( # <3>
      aes(
        data_id = interaction(fecha, param), tooltip = label, # <4>
        hover_nearest = TRUE
      ),
...
lista_figura_evolucion_lab <- map(  # <12>
  param_unid_v, f_figura_evolucion_lab  # <12>
)  # <12>
```
...
g_log <- function(g, etq) { # <1>
  g +
    ...
    annotate(
      geom = "richtext", x = I(0), y = I(1), hjust = 0,
      vjust = 1, size = texto_tamaño, label = etq,
      label.color = NA, fill = c11, family = "jet"
    ) +
...
f_gg <- function(eje_x, eje_y) { # <1>
  ...
  g <- ggplot(e, aes(x, y)) +
    geom_point( # <3>
      size = punto_tamaño, alpha = punto_transparencia, # <3>
      color = c2 # <3>
    ) + # <3>
    coord_cartesian(clip = "off") +
  ...
  if (eje_x == "secchi" | eje_y == "secchi") { # <5>
