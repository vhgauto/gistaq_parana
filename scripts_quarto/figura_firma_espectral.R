...
d_typst <- e |>
  filter(fecha == max(fechas_gis_v))

fig_evo_gis_label_typst <- c(
  "Chaco", rep("", length(unique(d_typst$p)) - 2), "Corrientes"
)

colores_gis_typst <- colorRampPalette(
  colors = c(c1, c4, c2))(length(unique(d_typst$p))
)

midpoint_gis_typst <- median(unique(d_typst$p))

g_typst <- ggplot(d_typst, aes(banda, reflect, group = p, color = p, fill = p)) +
  geom_line(linewidth = 1, alpha = .8, show.legend = FALSE) +
  ...
  scale_fill_gradient2(
    low = c1, mid = c4, high = c2, midpoint = midpoint_gis_typst,
    breaks = unique(d_typst$p),
    labels = fig_evo_gis_label_typst
  ) +
  scale_color_gradient2(
    low = c1, mid = c4, high = c2, midpoint = midpoint_gis_typst,
    breaks = unique(d_typst$p),
  ...
  theme(
    ...
    axis.title.x = element_text(family = "ubuntu", margin = margin(t = 3)),
    axis.title.y = element_markdown(
      family = "ubuntu", margin = margin(r = 10)
    ),
    axis.text = element_text(family = "jet", color = c7),
    axis.text.y = element_text(hjust = 1, margin = margin(r = 2)),
    axis.text.x = element_text(margin = margin(t = 2)),
...
ggsave(
  plot = g_typst,
  filename = paste0(getwd(), "/figuras_typst/firma_espectral.png"),
  width = 15,
  height = 15,
  units = "cm"
)
